# macOS Remediation Guide — Shai-Hulud / Mini Shai-Hulud

This guide is for developers who ran `audit.js` on a Mac and got hits — either
in their projects (`package-json` / `lockfile` findings) or on the machine
itself (persistence hooks). It walks the safe order of operations, OS-specific
...
> revoke any tokens.** The `gh-token-monitor` daemon planted by Shai-Hulud
> polls GitHub roughly every 60 seconds; if it sees a watched token get
> revoked it executes `rm -rf $HOME`. Sources: [Wiz][wiz],
> [Expel][expel], [Arctic Wolf][arctic], [Mini Shai-Hulud Scanner][scanner].
...
Re-run `node audit.js .` against any repo that has had **`npm install`**,
**`npm ci`**, **`pnpm install`**, or **`yarn install`** run inside the
exposure window of **29 April 2026 – 13 May 2026** (the Mini Shai-Hulud
publish window). Also check **CI build logs** for installs in that window —
agent caches can hold a resolved-but-not-yet-cleaned compromised version.

## 0c. Decision tree

Use this to pick the section to read.
...
$ node audit.js ~
                ...
                │       → Section 4 (repo-only remediation). No machine wipe.
                │
                └── Persistence hits ≥ 1     (regardless of repo findings)
                        → Section 1 (contain) → 2 (rotate) → 3 (wipe/reinstall)
...
### 1a. Disconnect from the network

Don't pull the plug abruptly if you're mid-write to disk — close editors first.
Then:

```sh
networksetup -setairportpower en0 off            # Wi-Fi off
# Unplug Ethernet by hand. Avoid `ifconfig down`; you'll need network later.
```

This stops the daemon from phoning home and prevents OIDC/token tests from
landing during your triage. The local `rm -rf` watchdog is **not** disarmed by
disconnecting — it only checks GitHub when it has connectivity. So while
offline you can safely inspect persistence without risking the wiper, as long
as you don't touch tokens.

### 1b. Take an APFS snapshot for forensics

```sh
tmutil localsnapshot                              # Time Machine local snapshot
tmutil listlocalsnapshots /                       # confirm
```

This is free, instant, and read-only. Don't rely on Time Machine remote backup
yet — Time Machine may have backed up the malicious files. Keep this snapshot
until you've reinstalled.

### 1c. Inventory persistence (read-only)

Run the bundled script (it only reads, never deletes):

```sh
bash scripts/triage-macos.sh
```

Or do it by hand. Known IOC paths (variants exist; check all):

```sh
ls -la ~/Library/LaunchAgents/ | grep -Ei 'token|gh-token|github|tanstack'
ls -la /Library/LaunchAgents/ 2>/dev/null
ls -la /Library/LaunchDaemons/ 2>/dev/null
launchctl list | grep -Ei 'token|github|gh-token'

# IDE artefacts
ls -la ~/.claude/ 2>/dev/null
ls -la ~/.vscode/ 2>/dev/null
grep -l 'router_runtime\|router_init\|fileURL' ~/.vscode/tasks.json 2>/dev/null

# Worm payload on disk
mdfind -name router_init.js 2>/dev/null
mdfind -name router_runtime.js 2>/dev/null
mdfind kMDItemFSName == 'setup.mjs' 2>/dev/null | grep -E '/.claude/|/.vscode/'

# Login items / cron / per-user systemd (rare on macOS)
osascript -e 'tell application "System Events" to get the name of every login item'
crontab -l 2>/dev/null
```

Known IOC file names (Wiz, Expel, audit.js IOC list):

| Path | Notes |
|---|---|
| `~/Library/LaunchAgents/com.user.gh-token-monitor.plist` | Wiz-documented variant |
| `~/Library/LaunchAgents/com.github.token-monitor.plist` | audit.js IOC list |
...
Order matters. Stop the LaunchAgent first so it can't react during file
deletion:

```sh
# Replace the label/path with what your `launchctl list` actually showed.
...
# Verify
launchctl list | grep -Ei 'token|gh-token'
```
...
mv ~/Library/LaunchAgents/com.user.gh-token-monitor.plist ~/quarantine-shai-hulud/ 2>/dev/null
mv ~/.claude ~/quarantine-shai-hulud/dot-claude 2>/dev/null
# Don't blindly mv ~/.vscode — back up tasks.json only:
...
untrusted until you've reinstalled.

**Scoping rule:** anything reachable as an environment variable, file, or
keychain entry from the compromised user account (or any CI build agent that
pulled an affected version) is in scope. When in doubt, rotate it. Also
review your GitHub **org audit log** for unexpected workflow runs / token
uses **since 29 April 2026**.
...
4. **Cloud creds** — AWS access keys + IAM roles; GCP service-account JSON;
   ...
   HashiCorp Vault, Kubernetes service-account tokens.
5. **CI/CD secrets** — GitHub Actions repository + organisation secrets,
   CircleCI contexts, GitLab CI variables.
6. **Anything in dotfiles** — grep your shell rc, `~/.npmrc`, `~/.netrc`,
   `~/.aws/credentials`, any `.env*` files for tokens before you destroy
   them.

The Wiz, Carthage, and Phoenix Security write-ups all stress: **assume every
secret accessible from the compromised user account is exposed.** Errors of
omission cost more than the rotation work.

---

## 3. Wipe and reinstall macOS

Recovery → Erase All Content and Settings is the cleanest path on modern
Macs. Two things to remember:

- **Do not restore `~/Library` or dotfiles** from your post-infection backup.
  Restore documents/photos by file, not by Migration Assistant.
- Reinstall apps from the Mac App Store / vendor sites, not from a backup.
- Rotate FileVault keys / re-encrypt as part of the fresh install.
...
## 4. Repo-level remediation (no host wipe needed)
...
# Outbound to known C2 should resolve nowhere / be NXDOMAIN.
dig +short git-tanstack.com
dig +short seed1.getsession.org
dig +short filev2.getsession.org

# pf rule to block C2 DNS at the host (optional, defence-in-depth):
sudo sh -c 'echo "0.0.0.0 git-tanstack.com" >> /etc/hosts'
sudo sh -c 'echo "0.0.0.0 seed1.getsession.org" >> /etc/hosts'
sudo sh -c 'echo "0.0.0.0 seed2.getsession.org" >> /etc/hosts'
sudo sh -c 'echo "0.0.0.0 seed3.getsession.org" >> /etc/hosts'
sudo sh -c 'echo "0.0.0.0 filev2.getsession.org" >> /etc/hosts'
sudo dscacheutil -flushcache
```

---

## 6. Hardening before the next install / release
...
`scripts/triage-macos.sh` is therefore **read-only**. It lists what to do; the
